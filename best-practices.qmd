...
You can do this by:
- Opening a text editor (nano, vim, VS Code, etc.)
- Pasting the script
- Saving as `create_project_structure.sh`
...
1.  **Never modify raw data:** Keep `data/raw/` read-only, always work with copies in `data/processed/`
2.  **Number your scripts:** Prefix with numbers (01\_, 02\_) to show analysis order
3.  **Use relative paths:** Scripts should work regardless of where the project is located
4.  **One result, one file:** Don't overwrite results; create new files with dates/versions
5.  **Document everything:** README files in each major directory explaining contents
:::
...
-   Set up your next project with the [standard folder structure](#project-folder-structure)
